# rim - 標準ライブラリのみで作る深層学習フレームワーク

**rim**は、Pythonの標準ライブラリのみを使用して実装する深層学習フレームワークです。
外部依存なしで、自動微分、ニューラルネットワーク、最適化アルゴリズムなどの深層学習の基本要素を理解し実装することを目的としています。

## 特徴

- **ゼロ依存**: NumPy、PyTorchなどの外部ライブラリを使わず、純粋なPythonで実装
- **教育的**: 深層学習の仕組みを理解するための透明な実装
